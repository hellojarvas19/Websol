...
1. Open `/start` → Click **⚙️ Settings** → **🔔 Notification Filters**

### DEX Filters
Toggle notifications for specific platforms:
- ✅/❌ **Pump.fun** - Bonding curve swaps and token mints
- ✅/❌ **PumpSwap** - Pump.fun AMM swaps
- ✅/❌ **Raydium** - All Raydium versions (AMM, CLMM, CPMM)
- ✅/❌ **Jupiter** - All Jupiter aggregator swaps
- ✅/❌ **SOL Transfers** - Native SOL transfers

### Minimum SOL Amount
Set a threshold to only receive notifications for larger swaps:
...
9. `(Optional)` Configure Solana Tracker API for accurate holders data:

```env
SOLANA_TRACKER_API_KEY=your_api_key_here
```

Get your API key from [https://www.solanatracker.io](https://www.solanatracker.io)
