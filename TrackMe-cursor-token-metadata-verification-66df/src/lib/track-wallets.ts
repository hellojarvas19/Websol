  ...
  public async setupWalletWatcher({ event, userId, walletId }: SetupWalletWatcherProps): Promise<void> {
    ...
    if (event === 'delete' && walletId) {
      ...
      if (refetchedWallet) {
        ...
        await WalletPool.withLock(async () => {
          const existingWalletIndex = WalletPool.findWalletIndex(refetchedWallet.address)
          ...
          if (existingWalletIndex !== -1) {
            ...
            if (subscriptionId !== undefined) {
              ...
              try {
                ...
                console.error(`Failed to remove listener for ${refetchedWallet.address}:`, err)
              }
            ...
            if (refetchedWallet.userWallets.length === 0) {
              ...
              WalletPool.removeWallet(existingWalletIndex)
            } else {
              // Update the wallet with the new userWallets if users remain
              WalletPool.updateWallet(existingWalletIndex, refetchedWallet)
            }
          }

          await this.updateWallets(WalletPool.wallets)
        })
