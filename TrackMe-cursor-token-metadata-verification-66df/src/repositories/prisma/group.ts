...
export class PrismaGroupRepository {
  ...
  /** Get group chat IDs for posting BUY alerts (by user IDs that own the wallet) */
  public async getGroupIdsByUserIds(userIds: string[]): Promise<{ id: string; userId: string }[]> {
    ...
    try {
      ...
      console.log('GET_GROUP_IDS_BY_USER_IDS', error)
      return []
    }
  }
