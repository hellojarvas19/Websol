...
function isRelevantTransaction(logs: Logs): { isRelevant: boolean; program: TestSwapType } {
  ...
  // Check PumpFun programs first (most specific)
...
export const parseTransactions = async (swap: TestSwapType) => {
  try {
    ...
    // Early exit if no transactions found (except for pumpfun_amm which is handled separately)
    if (transactions.length === 0 && swap !== 'pumpfun_amm') {
      console.log('NO TRANSFER INSTRUCTIONS FOUND')
      return
    }

    // Find the raydium/jupiter transfer instruction
    let raydiumTransfer: any = null
    
    if (transactions.length > 2) {
      // For complex swaps, try to find the matching transfer
      raydiumTransfer = transactions.find((t: any) => t?.info?.destination === transactions[0]?.info?.source)
      
      // Fallback: Look for the last transfer that's different from the first
      if (!raydiumTransfer) {
        raydiumTransfer = transactions.find((t: any, idx: number) => 
          idx > 0 && t?.info?.source !== transactions[0]?.info?.source
        )
      }
      
      // Final fallback: Use the last transaction
      if (!raydiumTransfer) {
        raydiumTransfer = transactions[transactions.length - 1]
      }
    ...
    if (!raydiumTransfer && swap !== 'pumpfun_amm') {
      console.log('NO RAYDIUM TRANSFER - transactions:', transactions.length)
      return
    }
